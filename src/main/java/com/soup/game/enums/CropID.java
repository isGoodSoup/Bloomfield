...
public enum CropID implements Item {
    WHEAT(Seasons.SPRING, "crop.wheat", 8, 5, 4, true),
    CABBAGE(Seasons.AUTUMN, "crop.cabbage", 8, 10, 6, false),
    CORN(Seasons.WINTER, "crop.corn", 16, 15, 6, false),
    CARROT(Seasons.SPRING, "crop.carrot", 8, 20, 10, false),
    POTATO(Seasons.WINTER, "crop.potato", 16, 15, 12, true),
    TOMATO(Seasons.SUMMER, "crop.tomato", 16, 15, 12, true),
    STRAWBERRY(Seasons.SUMMER, "crop.strawberry", 10, 16, 16, true),
    APPLE(Seasons.SPRING, "crop.apple", 32, 20, 16, false),
    GRAPE(Seasons.WINTER, "crop.grape", 16, 25, 24, false),
    PUMPKIN(Seasons.AUTUMN, "crop.pumpkin", 32, 25, 32, true);
     ...
     */
    CropID(Seasons season, String name, int yield, int days, int value, boolean canRegrow) {
        this.season = season;
        this.name = name;
        this.yield = yield;
        this.days = days;
    ...
    public static CropID random(Seasons currentSeason) {
        ...
        if (seasonalCrops.length == 0) {
            // fallback in case no crops exist for this season
